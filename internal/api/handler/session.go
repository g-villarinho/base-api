...
func (h *SessionHandler) Logout(c echo.Context) error {
	...
	if err := h.sessionService.DeleteSessionByID(c.Request().Context(), echoctx.GetUserID(c), echoctx.GetSessionID(c)); err != nil {
		if errors.Is(err, domain.ErrSessionNotFound) {
			logger.Warn("cannot logout: session not found", slog.String("session_id", echoctx.GetSessionID(c).String()))
			return NotFound(c, "SESSION_NOT_FOUND", "session not found")
		}

		log.Error("logout", slog.Any("error", err))
		return echo.ErrInternalServerError
	}
